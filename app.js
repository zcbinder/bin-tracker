...
async function init() {
  ...
  // Event delegation — document-level so modal forms are also captured
...
function updateBottomNav() {
  ...
  const map = { home: 'bnav-home', search: 'bnav-search', print: 'bnav-print' };
  const active = map[state.view];
...
async function renderHome() {
  ...
  const totalBins = locations.reduce((sum, l) => sum + (l.bins?.[0]?.count ?? 0), 0);

  let html = `
    <div class="page-header">
      <div class="page-header-left">
        <h1 class="page-title">My Storage</h1>
      </div>
    ...
    <div class="stats-banner">
      <div class="stat-card">
        ...
        <div>
          <div class="stat-card-value">${locations.length}</div>
          <div class="stat-card-label">Location${locations.length !== 1 ? 's' : ''}</div>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div>
          <div class="stat-card-value">${totalBins}</div>
          <div class="stat-card-label">Total Bin${totalBins !== 1 ? 's' : ''}</div>
        </div>
      </div>
    </div>
  `;
    ...
    html += `
      <div class="location-card" data-action="navigate" data-path="/location/${loc.id}">
        <div class="location-card-top">
          <div class="location-card-top-bg" style="background:${esc(loc.color)}"></div>
          <div class="location-icon-wrap" style="background:${hexWithAlpha(loc.color,.18)}">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="${esc(loc.color)}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
          </div>
        </div>
        <div class="location-card-body">
          <div class="location-card-header">
            ...
            <div class="location-card-actions" onclick="event.stopPropagation()">
              <button class="btn-icon" style="width:30px;height:30px" data-action="edit-location" data-id="${loc.id}" title="Edit">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
              </button>
              <button class="btn-icon" style="width:30px;height:30px;color:var(--danger)" data-action="delete-location" data-id="${loc.id}" data-name="${esc(loc.name)}" title="Delete">
          ...
          <div class="location-card-desc">${esc(loc.description) || '&nbsp;'}</div>
          <div class="location-footer">
            ...
            <span class="location-prefix-badge" style="background:${hexWithAlpha(loc.color,.12)};color:${esc(loc.color)}">${esc(loc.prefix)}</span>
          </div>
        </div>
      </div>
    `;
  ...
  for (const bin of bins) {
    ...
    html += `
      <div class="bin-card" data-action="navigate" data-path="/bin/${bin.id}">
        <div class="bin-card-color-bar" style="background:${esc(loc.color)}"></div>
        <div class="bin-card-inner">
          ...
          <div class="bin-card-label">${esc(bin.label) || '<span style="color:var(--text-muted);font-style:italic;font-weight:400">Unlabeled</span>'}</div>
...
async function renderBin(binId) {
  ...
  // Hero card
  html += `
    <div class="bin-detail-hero">
      <div class="bin-detail-hero-bar" style="background:${esc(color)}"></div>
      <div class="bin-detail-hero-body">
      ...
      <div class="bin-detail-actions">
        ...
        <button class="btn btn-secondary btn-sm" data-action="show-qr" data-bin-id="${binId}">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="4" height="4"/></svg>
          QR Code
        </button>
